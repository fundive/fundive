...
// speed, slight horizontal sine wobble) so they don't form visible columns the
// way a fixed grid would. Drawn as a translucent fill plus a brighter stroke so
// they read as bubbles rather than solid balls. The canvas fills its parent
// (absolute inset-0) and measures off itself, so the dashboard just drops it in.
  ...
  useEffect(() => {
    ...
    function draw(ts: number) {
      ...
      for (const b of bubbles) {
        ...
        // the bottom edge with new attributes — keeps the pattern non-periodic.
