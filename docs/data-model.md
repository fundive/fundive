...
| `enters_water` (NOT NULL, default true) | shared. False when nobody who registers goes in the water — an EFR/CPR class, an equipment course, a BBQ. The Logistics board then lists those registrants as **non-divers** rather than divers, which is the split the shop's insurer asks for; see [admin.md](./admin.md#divers-and-non-divers). Ask it through `eventEntersWater()` (`src/lib/participants.ts`), never raw: an adventure answers no by kind whatever the column says. Set on the admin event form, and carried in every `create_events_with_relations` payload for the same NOT NULL reason as `has_transport` (20260925100000) |
