...
whatever `.env.local` holds — which, for a fresh clone, is the local demo key
from `.env.example` rather than the key this particular stack is running with —
and every read through that client would fail auth. The lib helpers swallow the error and return `[]`, so
